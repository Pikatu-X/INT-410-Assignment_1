...
if __name__=='__main__':
    iris_data=pd.read_csv("iris.data",header=None) # 数据没有列名 `header=None`
    labels_codes=pd.Categorical(iris_data[4]).codes # 列表 0，1，2分别表示三个不同的类
    for i in range(150):
        iris_data.loc[i,4]=labels_codes[i] # 类别名称 ---> 0，1，2
    datalist=iris_data.values.tolist() # 提取样本数据存入列表[[] ... []]
    random.seed(17)
    random.shuffle(datalist)
    data_set=np.mat(datalist) # 类型转换 numpy matrix (150x5)，类别是第5个元素
