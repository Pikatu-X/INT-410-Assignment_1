...
class Data_process:  # Class for data pre-processing
    ...
    def load_data(self):  # Method to load the dataset and store them in a list
        with open(self.filename) as f:
            for line in f: # 整行读入 str类型
                text_lines = line.strip() # 去除字符串两端空白字符
    ...
    def numeric_n_name(self, nested_list):  # Method to separate the numeric data and class_names
        ...
        return num_list, class_list  # num_list 2维列表; class_list 列表
                                     # Numeric data can be converted into numpy array
...
def QDF_model(train_data, class_num):  # Modified function from QDF to obtain the required parameters for MQDF
    ...
    for i in range(class_num):  # For all classes
        ...
        train_data[i] = np.array(train_data[i], dtype=np.float64) # Convert the ith data to a matrix & use 64-bit precision
        # mean.append(train_data[i].mean(axis=0).T)  # Store the mean for all features in ith class
        mean.append(np.mean(train_data[i], axis=0))
        cov_matrix.append(np.cov(train_data[i].T))  # Store the covariance matrix of ith class
...
def MQDF2_model(cov, d, k, class_num):  # Function to obtain the trained parameters of MQDF2
    ...
    for i in range(class_num):  # For all classes
        ...
        delta[i] = sum(eigvals[k:]) / (d - k) # Compute delta as the mean of minor values. Tip: sum(eigvals[int(k):]) / (d - k)
        ###############################################################################################################
    ...
    for sample in np_x: # For the number of test samples
        ...
        for i in range(class_num): # For all classes
            ...
            for j in range(int(k)): # For the range of k
                # euc_dis[j] = ((eigenvector[i][:, j].T * (test_x - mean[i].reshape((4,1))))[0, 0]) ** 2
                # ma_dis[j] = (((test_x - mean[i]).T * eigenvector[i][:, j])[0, 0]) ** 2
                euc_dis[j] = ((eigenvector[i][:, j] * (test_x - mean[i].reshape(-1, 1)))[0, 0]) ** 2
                ma_dis[j] = (((test_x - mean[i].reshape(-1, 1)).T * eigenvector[i][:, j].reshape(-1, 1))[0, 0]) ** 2
...
def MQDF1_model(cov, d, class_num, h):  # Function to obtain the trained parameters of MQDF1
    ...
    for i in range(class_num):  # For all classes
        ...
        #                                   YOU NEED FILL FOLLOWING CODES:
        covMat = cov[i] # Get the covariance matrix of ith class
        eigvals, eigvecs = np.linalg.eig(covMat) # Obtain eigenvalues and eigenvectors from the cov. matrix. Tip: use np.linalg.eig()
        ...
        delta[i] = eigvals.mean()# Compute delta as the mean of minor values.
        ###############################################################################################################
    return eigenvalue, eigenvector, delta
...
if __name__ == '__main__':
    print('starting...')
    import os # Python标准库OS模块
    os.environ['MKL_NUM_THREADS'] = '1' # os.environ获取一个环境变量字典，包含操作系统中定义的所有环境变量
    os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"
    iris = Data_process() # class Data_process
    irist_data = iris.load_data() # 列表 [[iris样本]...[]]
    div_data = iris.numeric_n_name(irist_data) # 元组 ([num_list], [class_list])
                                               # Separate numeric dataset and class names
    init_data = iris.shuffle() # Shuffle the dataset
    five_data = iris.separate_data() # 元组 ([data1], [data2], ..., [data5])
                                     # Divide the dataset for 5-fold cross validation
    # Predefined parameters
    classNum = 3  # Three classes: setosa, versicolor, virginica
    d = len(div_data[0][0]) # len(num_list[0])
                            # Feature numbers = 4
    ...
    ###############################################################################################################
    #                                   YOU NEED FILL FOLLOWING CODES:
    for h in range(80): # find a range of h for test
    ###############################################################################################################
        mqdf_sum_avg_acc = 0
        cnt = 1
        # 总数据集分为5组数据，依次选1组为test,4组为train，共5次
        for index in range(len(five_data)): # 5-fold cross validation
            total_subset = iris.combine_train(index, five_data) # ([train], [test]) train含4组数据, test含1组数据
                                                                # Index denotes the array for testing
            sep_dataset = iris.separate_class(total_subset[0])  # train数据集分组 ([setosa], [versicolor], [virginica])
                                                                # Return separated train datasets by three classes setosa, versicolor, virginica
            sep_data = [sep_dataset[0], sep_dataset[1], sep_dataset[2]] # [[setosa], [versicolor], [virginica]]
                                                                        # Nested list of the three datasets
            # Only extract the numeric data from the datasets
            np_se = np.array(iris.numeric_n_name(sep_data[0])[0]) # numeric_n_name return num_list, class_list
    ...
    ###############################################################################################################
    #                                   YOU NEED FILL FOLLOWING CODES:
    for k in range(d): # find a range of k for test
    ##############################################################################################################
        mqdf_sum_avg_acc = 0
        cnt = 1
        for index in range(len(five_data)): # 5-fold cross validation
