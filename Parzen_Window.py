...
def twoD_plot(filename): # To check the general properties of the dataset in 2D (Additional task)
    data = pd.read_csv(filename, names=["sepal length", "sepal width", "petal length", "petal width", "class"]) # 读取数据并且为每一列数据添加名称标签
    data.head(5) # 打印数据集的前5行
    data.describe() # 生成数据集的描述性统计信息，包括均值、标准差、最小值、最大值等，用于了解数据的分布情况
    data.groupby('class').size() # 对数据集按照类别 class 进行分组，并统计每个类别的样本数量。
    sns.pairplot(data, hue="class", height=2, palette='colorblind') # 使用Seaborn库的 pairplot 函数绘制了数据集中各个特征两两之间的关系图，并使用类别信息进行着色。这有助于观察不同特征之间的相关性以及不同类别的分布情况。
    plt.show()
...
def fourD_plot(p1, p2, p3, p4): # To check the general properties of the dataset in 4D (Additional task)
    ...
    img = ax.scatter(x, y, z, c=c, cmap=plt.hot()) # The 4D datasets will be shown in the 3D coordinate with color gradient, c (petal width) --- colour
    fig.colorbar(img)
    # Add axis
...
def multivariate_normal_kernel(x, xi, h, cov): # Kernel function for datasets higher than 1D
    ...
    numer = np.exp(-0.5 * np.dot(np.dot(u,inv_cov),u.T)) # Numerator of the multivariate gaussian distribution, math is pow(e, ( -0.5 * u * inv_cov * u.T))
    denom = np.sqrt((2 * np.pi)**len(x) * det_cov) # Denominator of the multivariate gaussian distribution, math is pow(pow(2*pi, len(x)) * det_cov, 1/2)
...
class Data_process: # Class for data pre-processing
    ...
    def shuffle(self): # Method to shuffle the stored dataset 打乱数据集中样本顺序
...
if __name__ == '__main__':
    ...
    iris.data_analyzer(div_data[0]) # Check the general properties of the dataset 输出原始数据的图
    ...
    for h in np.arange(0.2, 3, 0.1): # Find the optimal kernel via Changing the h value
        ...
        for index in range(len(five_data)): # 5-fold Cross-Validation
            ...
            prior_prob = iris.prior_prob(sep_dataset) # Calculate the prior probabilties of three classes 训练集train
